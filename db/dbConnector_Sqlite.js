...
async function checksentemails(user_id) {
  try {
      ...
      // console.log(user_id);
      // 获取用户发送的所有邮件ID
      const emailIds = await clientRedis.lRange('sentemails:' + user_id.toString(), 0, -1);
      ...
      console.error('Error finding sent emails:', error);
      return [];
  } finally {
      await clientRedis.quit();
  }
...
async function checkContacts(user_id) {
  try {
      ...
      // 读取并反序列化
      const storedArrayString = await clientRedis.hGet('users:'+user_id, 'contacts');
      const storedArray = JSON.parse(storedArrayString);
      // 获取每个联系人的详细信息
      let contactsDetails = [];
      for (const contactId of storedArray) {
          let contactData = await clientRedis.hGetAll('users:' + contactId);
          if (Object.keys(contactData).length > 0) {
              contactsDetails.push(contactData);
          }
      }
